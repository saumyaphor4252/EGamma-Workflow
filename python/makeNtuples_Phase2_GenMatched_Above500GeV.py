...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create debug ntuples for E/Gamma trigger objects from Phase2 data with Gen matching (ET ≥ 500 GeV)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 makeNtuples_Phase2_GenMatched_Above500GeV.py input.root -o output.root
  python3 makeNtuples_Phase2_GenMatched_Above500GeV.py --output output.root input.root
  python3 makeNtuples_Phase2_GenMatched_Above500GeV.py --max-events 100 -o output.root input.root
  python3 makeNtuples_Phase2_GenMatched_Above500GeV.py --input-dir /path/to/files/ -o output.root
  python3 makeNtuples_Phase2_GenMatched_Above500GeV.py --input-pattern "*.root" -o output.root
  python3 makeNtuples_Phase2_GenMatched_Above500GeV.py --help
        """
    )
    ...
    if args.input_file:
        ...
        if "eos" in args.input_pattern:
            ...
            try:
                ...
                input_files = glob.glob(args_input_pattern)
        else:
            input_files = glob.glob(args.input_pattern)
    ...
    print("⚙️  ET selection: keeping only trigger objects with ET ≥ 500 GeV")
    if verbose:
    ...
    tree = ROOT.TTree("egHLTTree", "EGamma Tree with Gen Matching (ET ≥ 500 GeV)")
    ...
    gen_matched = ROOT.std.vector("int")()  # 1 if matched, 0 if not
    ...
    for i, evt in enumerate(events):
        ...
        try:
            ...
            if genparts_handle.isValid():
                ...
                if verbose:
                    print(
                        f"  ⚠ Gen particle collection '{gen_label}' "
            ...
            try:
                ...
                if handle.isValid():
                    ...
                    if egobjs.size() > 0:
                        ...
                        # Process each object (ET ≥ 500 GeV only)
                        for j, obj in enumerate(egobjs):
                            obj_et = obj.et()
                            if obj_et < 500.0:
                                if verbose:
                                    print(
                                        f"    → Skipping object {j} with "
                                        f"ET={obj_et:.1f} GeV (< 500 GeV)"
                                    )
                                continue
                            ...
                            if genparts is not None:
                                ...
                                if best_match is not None:
                                    ...
                                    if verbose:
                                        print(
                                            f"    → ET≥500 object {j} matched to gen "
                                            f"pt={best_pt:.2f}, "
                                            f"eta={best_match.eta():.3f}, "
                                            f"phi={best_match.phi():.3f}, "
                                            f"dR={this_dr:.4f}"
                                        )
                                else:
                        ...
                        if filtered_count > 0:
                            if verbose:
                                print(
                                    f"    → Stored {filtered_count} objects with ET ≥ 500 GeV "
    ...
    print("Gen matching information is stored in gen_* branches (ET ≥ 500 GeV only).")
