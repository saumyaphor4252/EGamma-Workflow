...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create debug ntuples for E/Gamma trigger objects from Phase2 data with Gen matching (EB only, |eta| < 1.5)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 makeNtuples_Phase2_GenMatched_EB.py input.root -o output.root
  python3 makeNtuples_Phase2_GenMatched_EB.py --output output.root input.root
  python3 makeNtuples_Phase2_GenMatched_EB.py --max-events 100 -o output.root input.root
  python3 makeNtuples_Phase2_GenMatched_EB.py --input-dir /path/to/files/ -o output.root
  python3 makeNtuples_Phase2_GenMatched_EB.py --input-pattern "*.root" -o output.root
  python3 makeNtuples_Phase2_GenMatched_EB.py --help
        """
    )
    ...
    print("🏠 EB selection: keeping only trigger objects with |eta| < 1.5")
    if verbose:
    ...
    tree = ROOT.TTree("egHLTTree", "EGamma Tree with Gen Matching (EB only)")
            ...
            try:
                ...
                if handle.isValid():
                    ...
                    if egobjs.size() > 0:
                        ...
                        # Process each object (EB only: |eta| < 1.5)
                        ...
                        if filtered_count > 0:
                            if verbose:
                                print(
                                    f"    → Stored {filtered_count} EB objects "
    ...
    print("Gen matching information is stored in gen_* branches (EB only).")
