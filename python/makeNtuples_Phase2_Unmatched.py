...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create debug ntuples for E/Gamma trigger objects from Phase2 data that are NOT gen-matched",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
This script is the complement of makeNtuples_Phase2_GenMatched.py:
it only stores trigger objects that do NOT have a matching gen electron.

Examples:
  python3 makeNtuples_Phase2_unmatched.py input.root -o output.root
  python3 makeNtuples_Phase2_unmatched.py --output output.root input.root
  python3 makeNtuples_Phase2_unmatched.py --max-events 100 -o output.root input.root
  python3 makeNtuples_Phase2_unmatched.py --input-dir /path/to/files/ -o output.root
  python3 makeNtuples_Phase2_unmatched.py --input-pattern "*.root" -o output.root
  python3 makeNtuples_Phase2_unmatched.py --help
        """
    )
    ...
    parser.add_argument(
        ...
        help="Label for gen particle collection (default: genParticles)",
    )
    parser.add_argument(
        "--gen-process",
        default="",
        help="Process name for gen particle collection (default: empty)",
    )
    parser.add_argument(
        "--max-dr",
        type=float,
        default=0.1,
        help="Maximum deltaR for gen matching (default: 0.1)",
    )
    parser.add_argument(
        "--gen-pid",
        type=int,
        default=11,
        help="PDG ID for gen particles to match (default: 11 for electrons)",
    )
    ...
    print(f"🧬 Gen matching: PID={gen_pid}, max_dR={max_dr}, label={gen_label}")
    print("❌ Storing ONLY trigger objects without a gen match")
    if verbose:
    ...
    tree = ROOT.TTree("egHLTTree", "EGamma Tree with Unmatched Trigger Objects")
    ...
    # Gen matching variables (will be zero / dummy for unmatched)
    gen_matched = ROOT.std.vector("int")()  # 0 for all stored objects
    ...
    for i, evt in enumerate(events):
        ...
        print(f"\n=== Event {i} ===")
        print(
            f"Run: {evt.eventAuxiliary().run()}, "
            f"Lumi: {evt.eventAuxiliary().luminosityBlock()}, "
            f"Event: {evt.eventAuxiliary().event()}"
        )
        ...
        try:
            ...
            if genparts_handle.isValid():
                ...
                if verbose:
                    print(
                        f"  ⚠️  Gen particle collection '{gen_label}' "
        ...
        for collection_info in collections_to_try:
            ...
            print(
                f"\nTrying collection: {col_name} with label: {label}"
            ...
            try:
                ...
                if handle.isValid():
                    ...
                    if egobjs.size() > 0:
                        ...
                        # Process each object: keep ONLY those without a gen match
                        for j, obj in enumerate(egobjs):
                            ...
                            # We want ONLY unmatched objects
                            if best_match is not None:
                                if verbose:
                                    this_dr = ROOT.reco.deltaR(
                                        obj_eta,
                                        obj_phi,
                                        best_match.eta(),
                                        best_match.phi(),
                                    ...
                                    print(
                                        f"    → Skipping matched object {j}: "
                                        f"pt={best_pt:.2f}, dR={this_dr:.4f}"
                                    )
                                continue

                            # Unmatched object passes selection
                            filtered_count += 1
                            ...
                            eg_nrClus.push_back(
                                obj.superCluster().clusters().size()
                            )
                            eg_rawEnergy.push_back(
                                obj.superCluster().rawEnergy()
                            )
                            eg_phiWidth.push_back(
                                obj.superCluster().phiWidth()
                            )
                            eg_seedId.push_back(
                                obj.superCluster().seed().seed().rawId()
                            )
                            eg_seedDet.push_back(
                                obj.superCluster().seed().seed().det()
                            )

                            # HLT variables - try different naming conventions
                            eg_sigmaIEtaIEta.push_back(
                                obj.var(
                                    "hltEgammaClusterShapeUnseeded_"
                                    "sigmaIEtaIEta5x5",
                                    0,
                                )
                            )
                            eg_ecalPFIsol_default.push_back(
                                obj.var(
                                    "hltEgammaEcalPFClusterIsoUnseeded", 0
                                )
                            )
                            eg_hcalPFIsol_default.push_back(
                                obj.var(
                                    "hltEgammaHcalPFClusterIsoUnseeded", 0
                                )
                            )
                            eg_hgcalPFIsol_default.push_back(
                                obj.var(
                                    "hltEgammaHGCalPFClusterIsoUnseeded", 0
                                )
                            )
                            eg_trkIsolV0_default.push_back(
                                obj.var(
                                    "hltEgammaEleGsfTrackIsoUnseeded", 0
                                )
                            )
                            eg_trkIsolV6_default.push_back(
                                obj.var(
                                    "hltEgammaEleGsfTrackIsoV6Unseeded", 0
                                )
                            )
                            eg_trkIsolV72_default.push_back(
                                obj.var(
                                    "hltEgammaEleGsfTrackIsoV72Unseeded", 0
                                )
                            )
                            eg_trkChi2_default.push_back(
                                obj.var(
                                    "hltEgammaGsfTrackVarsUnseeded_Chi2", 0
                                )
                            )
                            ...
                            eg_invESeedInvP.push_back(
                                obj.var(
                                    "hltEgammaGsfTrackVarsUnseeded_"
                                    "OneOESeedMinusOneOP",
                                    0,
                                )
                            )
                            eg_invEInvP.push_back(
                                obj.var(
                                    "hltEgammaGsfTrackVarsUnseeded_"
                            ...
                            eg_trkDEta.push_back(
                                obj.var(
                                    "hltEgammaGsfTrackVarsUnseeded_Deta", 0
                                )
                            )
                            ...
                            eg_sigma2uu.push_back(
                                obj.var(
                                    "hltEgammaHGCALIDVarsUnseeded_"
                                    "sigma2uu",
                                    0,
                                )
                            )
                            eg_sigma2vv.push_back(
                                obj.var(
                                    "hltEgammaHGCALIDVarsUnseeded_"
                                    "sigma2vv",
                                    0,
                            ...
                            eg_pms2_default.push_back(
                                obj.var(
                                    "hltEgammaPixelMatchVarsUnseeded_s2", 0
                                )
                            )
                            eg_hcalHForHoverE.push_back(
                                obj.var(
                                    "hltEgammaHGCALIDVarsUnseeded_"
                                    "hForHOverE",
                                    0,
                            ...
                            eg_l1TrkIsoCMSSW.push_back(
                                obj.var("hltEgammaHoverEUnseeded", 0)
                            )
                            eg_bestTrkChi2.push_back(
                                obj.var(
                                    "hltEgammaEleL1TrkIsoUnseeded", 0
                                )
                            )
                            eg_bestTrkDEta.push_back(
                                obj.var(
                                    "hltEgammaBestGsfTrackVarsUnseeded_Chi2",
                                    0,
                                )
                            )
                            eg_bestTrkDEtaSeed.push_back(
                                obj.var(
                                    "hltEgammaBestGsfTrackVarsUnseeded_Deta",
                                    0,
                                )
                            )
                            eg_bestTrkDPhi.push_back(
                                obj.var(
                                    "hltEgammaBestGsfTrackVarsUnseeded_"
                                    "DetaSeed",
                                    0,
                                )
                            )
                            ...
                            eg_hgcaliso_layerclus.push_back(
                                obj.var(
                                    "hltEgammaBestGsfTrackVarsUnseeded_"
                                    "ValidHits",
                                    0,
                            ...
                            eg_hgcaliso_layerclusem.push_back(
                                obj.var(
                                    "hltEgammaHGCalLayerClusterIsoUnseeded",
                                    0,
                                )
                            )
                            eg_hgcaliso_layerclushad.push_back(
                                obj.var(
                                    "hltEgammaHGCalLayerClusterIsoUnseeded_"
                                    "em",
                                    0,
                                )
                            )

                            # For unmatched objects, fill dummy gen info
                        ...
                        nr_objects.push_back(filtered_count)

                        if filtered_count > 0:
                            if verbose:
                                print(
                                    f"    → Stored {filtered_count} UNMATCHED objects "
                                    f"out of {egobjs.size()} total"
                                )
                            ...
                            if verbose:
                                print(
                                    "    → No unmatched objects in this collection"
                                )
    ...
    print("Only trigger objects WITHOUT a gen match are stored (gen_matched=0).")
