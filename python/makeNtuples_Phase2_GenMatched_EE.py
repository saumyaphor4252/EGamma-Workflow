...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create debug ntuples for E/Gamma trigger objects from Phase2 data with Gen matching (EE only, |eta| > 1.5)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 makeNtuples_Phase2_GenMatched_EE.py input.root -o output.root
  python3 makeNtuples_Phase2_GenMatched_EE.py --output output.root input.root
  python3 makeNtuples_Phase2_GenMatched_EE.py --max-events 100 -o output.root input.root
  python3 makeNtuples_Phase2_GenMatched_EE.py --input-dir /path/to/files/ -o output.root
  python3 makeNtuples_Phase2_GenMatched_EE.py --input-pattern "*.root" -o output.root
  python3 makeNtuples_Phase2_GenMatched_EE.py --help
        """
    )
    ...
    print("🏠 EE selection: keeping only trigger objects with |eta| > 1.5")
    if verbose:
    ...
    tree = ROOT.TTree("egHLTTree", "EGamma Tree with Gen Matching (EE only)")
            ...
            try:
                ...
                if handle.isValid():
                    ...
                    if egobjs.size() > 0:
                        ...
                        # Process each object (EE only: |eta| > 1.5)
                        ...
                        if filtered_count > 0:
                            if verbose:
                                print(
                                    f"    → Stored {filtered_count} EE objects "
    ...
    print("Gen matching information is stored in gen_* branches (EE only).")
