...
parser.add_argument("--blockFile", type=str, required=True, help="Text file containing one DAS block per line")
...
# ------------------------------------------------------------
# Query files from a DAS block
# ------------------------------------------------------------
def get_block_files(block):
    
  das_command = f'dasgoclient -query="file block={block}"'
...
for project_name, project_cfg in cfg["project"].items():
    ...
    all_input_files = []
    for block in blocks:
        print(f"Querying block: {block}")
        files = get_block_files(block)
        if len(files) == 0:
            print(f"WARNING: no accessible files found for {block}")
    ...
    print(f"Found {len(all_input_files)} accessible files")
